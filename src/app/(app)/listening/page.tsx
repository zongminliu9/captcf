...
import { EXAM_SPEC, LISTENING_SUBTYPES } from "@/lib/exam/config";
import { type MasteryState, masteryLabel } from "@/lib/mastery";
import { formatMinutes } from "@/lib/utils";

export const dynamic = "force-dynamic";

const SKILL = "listening" as const;
const spec = EXAM_SPEC[SKILL];

const SUBTYPE_LABELS: Record<(typeof LISTENING_SUBTYPES)[number], string> = {
  image_match: "Association d'images",
  short_dialogue: "Dialogue court",
  announcement: "Annonce et message",
  interview_excerpt: "Extrait d'entretien",
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:py-10">
      ...
      <h1 className="display mt-2 flex items-center gap-2 text-3xl">
        <Headphones className="h-7 w-7 text-navy" aria-hidden />
        {spec.labelFr}
      </h1>
      <p className="mt-2 max-w-2xl text-sm text-muted">
        Entraînez votre oreille sur des enregistrements courts et authentiques, de difficulté
        croissante. Chaque extrait n'est entendu qu'une seule fois, comme à l'examen.
      </p>
      ...
      <Card raised className="mt-6 flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
        ...
        <Button asChild variant="primary" size="lg" className="w-full sm:w-auto">
          <a href="/practice/start?mode=listening">
      ...
      <section className="mt-8" aria-labelledby="subtypes-heading">
        ...
        <p className="mt-1 text-sm text-muted">
          Les questions d'écoute couvrent plusieurs formats, du plus simple au plus exigeant.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {LISTENING_SUBTYPES.map((st, i) => {
